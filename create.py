...
for root, dirs, files in os.walk('.'):
    for filename in files:
        if root.find("Pods") == -1 and root.find(".git") == -1:
            ...
            if 'LMWebViewJavascriptBridge' in content:
                content = content.replace('LMWebViewJavascriptBridge', sys.argv[1])
                with open(filepath, 'w') as f:
                    f.write(content)

            if filename.find('LMWebViewJavascriptBridge') != -1:
                newfilename = filename.replace("LMWebViewJavascriptBridge", sys.argv[1])
...
for root, dirs, files in os.walk('.'):
    if root.find("Pods") == -1 and root.find(".git") == -1:
        for directory in dirs:
            if directory.find('LMWebViewJavascriptBridge') != -1:
                newdir = directory.replace("LMWebViewJavascriptBridge", sys.argv[1])
